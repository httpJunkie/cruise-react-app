...
const fetchPosts = (subreddit) => {
  return (dispatch) => {
    // Dispatch requestPosts action just before attempting to fetch data
    dispatch(requestPosts(subreddit))

    // Fetch data and dispatch recievePosts if no errors
    // Catch shouldn't be used to handle errors as 
    return fetch(`https://www.reddit.com/r/${subreddit}.json?limit=20`)
      .then(
...
// detrmine if we should fetch posts
